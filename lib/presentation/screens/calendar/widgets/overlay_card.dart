...
class _OverlayCardState extends State<OverlayCard> {
              ...
              ListTile(
                ...
                trailing: TextButton(
                  onPressed: () async {
                    ...
                    if (time == null) return;
                    final start = event.startAsUtc.copyWith(
                      hour: time.hour,
                      minute: time.minute,
                    );
                    if (start.isAfter(event.endAsUtc) ||
                        start == event.endAsUtc)
                      return;
                  ...
                  child: Text(
                    locale.formatTimeOfDay(
                      TimeOfDay.fromDateTime(event.startAsUtc),
              ...
              ListTile(
                ...
                trailing: TextButton(
                  onPressed: () async {
                    ...
                    if (time == null) return;
                    final end = event.endAsUtc.copyWith(
                      hour: time.hour,
                      minute: time.minute,
                    );
                    if (end.isBefore(event.startAsUtc) ||
                        end == event.startAsUtc)
                      return;
                  ...
                  child: Text(
                    locale.formatTimeOfDay(
                      TimeOfDay.fromDateTime(event.endAsUtc),
                      alwaysUse24HourFormat: use24,
                    ),
                  ),
                ),
  ...
  Future<DateTime?> _showDatePicker(DateTime first, DateTime last) async {
    return showDatePicker(context: context, firstDate: first, lastDate: last);
  }
